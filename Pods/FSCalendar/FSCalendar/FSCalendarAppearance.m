...
@property (strong, nonatomic) NSMutableDictionary *subtitle02Colors;
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        _subtitle02Colors = [NSMutableDictionary dictionaryWithCapacity:5];
        _subtitle02Colors[@(FSCalendarCellStateNormal)]      = [UIColor darkGrayColor];
        _subtitle02Colors[@(FSCalendarCellStateSelected)]    = [UIColor whiteColor];
        _subtitle02Colors[@(FSCalendarCellStateDisabled)]    = [UIColor lightGrayColor];
        _subtitle02Colors[@(FSCalendarCellStatePlaceholder)] = [UIColor lightGrayColor];
        _subtitle02Colors[@(FSCalendarCellStateToday)]       = [UIColor whiteColor];
...
- (void)setSubtitle02DefaultColor:(UIColor *)color
{
    if (color) {
        _subtitle02Colors[@(FSCalendarCellStateNormal)] = color;
    } else {
        [_subtitle02Colors removeObjectForKey:@(FSCalendarCellStateNormal)];
    }
    [self.calendar configureAppearance];
}
...
-(UIColor *)subtitle02DefaultColor
{
    return _subtitle02Colors[@(FSCalendarCellStateNormal)];
}
...
- (void)setSubtitle02TodayColor:(UIColor *)color
{
    if (color) {
        _subtitle02Colors[@(FSCalendarCellStateToday)] = color;
    } else {
        [_subtitle02Colors removeObjectForKey:@(FSCalendarCellStateToday)];
    }
    [self.calendar configureAppearance];
}
...
- (UIColor *)subtitle02TodayColor
{
    return _subtitle02Colors[@(FSCalendarCellStateToday)];
}
