...
@property (readonly, nonatomic) UIColor *colorForSubtitle02Label;
...
- (void)layoutSubviews
{
    ...
    if (_subtitle) {
        ...
        _subtitleLabel02.text = _subtitle02;
        if (_subtitleLabel02.hidden) {
            _subtitleLabel02.hidden = NO;
        }
...
//                                       (self.contentView.fs_height*5.0/6.0-height)*0.5+self.preferredTitleOffset.y - 5,
        ...
        _subtitleLabel.frame = CGRectMake(
                                          ...
                                          (_titleLabel.fs_bottom-self.preferredTitleOffset.y) - (_titleLabel.fs_height-_titleLabel.font.pointSize)+self.preferredSubtitleOffset.y + 6,
                                          self.contentView.fs_width,
                                          subtitleHeight
                                          );
        
        _subtitleLabel02.frame = CGRectMake(
                                          self.preferredSubtitle02Offset.x,
                                          (_subtitleLabel.fs_bottom-self.preferredSubtitleOffset.y) - (_subtitleLabel.fs_height-_subtitleLabel.font.pointSize)+self.preferredSubtitle02Offset.y + 16,
                                          self.contentView.fs_width,
                                          subtitleHeight
                                          );
    } else {
...
//    CGFloat diameter = MIN(self.bounds.size.height*5.0/6.0 + 10,self.bounds.size.width + 10);
...
- (void)configureAppearance
{
    ...
    if (_subtitle02) {
        textColor = self.colorForSubtitle02Label;
        if (![textColor isEqual:_subtitleLabel02.textColor]) {
            _subtitleLabel02.textColor = textColor;
        }
        titleFont = self.calendar.appearance.subtitleFont;
        if (![titleFont isEqual:_subtitleLabel02.font]) {
            _subtitleLabel02.font = titleFont;
        }
    }
...
- (UIColor *)colorForSubtitle02Label
{
    if (self.selected) {
        return self.preferredSubtitle02SelectionColor ?: [self colorForCurrentStateInDictionary:_appearance.subtitle02Colors];
    }
    return self.preferredSubtitle02DefaultColor ?: [self colorForCurrentStateInDictionary:_appearance.subtitle02Colors];
}
...
OFFSET_PROPERTY(preferredSubtitle02Offset, PreferredSubtitle02Offset, _appearance.subtitle02Offset);
...
- (void)setSubtitle02:(NSString *)subtitle
{
    if (![_subtitle02 isEqualToString:subtitle]) {
        BOOL diff = (subtitle.length && !_subtitle02.length) || (_subtitle02.length && !subtitle.length);
        _subtitle02 = subtitle;
        if (diff) {
            [self setNeedsLayout];
        }
    }
